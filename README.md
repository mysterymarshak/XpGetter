...
* **500 errors:** It may happen and I have retry logic for those situations. But in some cases retry logic doesn't help since the error itself is a bug. For test purposes I have created new steam account and when Im trying to get inventory history, instead of getting some empty or not success result, I always get 500 error. It's a **steam bug** and fixing it from my side is not relevant imo. So avoid using this program with new/test/empty accounts since it doesn't make any sense first of all, and second fighting with steam limitations for those accounts just sucks
